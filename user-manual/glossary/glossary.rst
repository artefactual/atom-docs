...
.. |caret-down-grey| image:: ../access-content/images/caret-down-grey.png
...
.. glossary::
  ...
  About page
    ...
    (see :ref:`Manage static pages <manage-static-pages>`).
  ...
  Access privilege
    ...
    :ref:`User roles <user-roles>`). See : :ref:`edit-user-permissions`.
  ...
  Add menu
    ...
    Only logged-in users can see the "Add" menu, which appears in the
    :term:`main menu` located in the :term:`header bar`. See: :ref:`main-menu`.
  ...
  Archival unit
    ...
    unit is the item. See also :term:`level of description`.

  Area header
    The title/heading that appears at the top of :term:`information areas
  ...
  Breadcrumb trail
    ...
    currently showing. At the top of:term:`archival institution` and
    :term:`authority record` view pages, the breadcrumb trail is used
    to provide a link back to the Browse page for each kind of record.
    Additionally, :term:`taxonomy` :term:`terms <term>` (such as
    :term:`places <place>` and :term:`subjects <subject>`) that are
  ...
  Browse menu
    ...
    page`. See: :ref:`navigate` and :ref:`Browse <browse>`; see also:
    :ref:`browse-menu`.
  ...
  Button block
    ...
    :term:`edit pages <edit page>`. See: :ref:`recurring-button-block`.
  ...
  Column header
    ...
    list pages and some browse results (e.g., :term:`subject` and :term:`place`
    browse pages), and give the name of the :term:`field` for whatever is
    being listed below. On some pages, the column headers include a
    |caret-down-grey| caret icon next to the column header :term:`field` name
    - when clicked, these icons will reverse the sort order of the data in the
    list based on that column (i.e., changing from A-Z to Z-A, or from most
    recently updated first to oldest update first).

    Column headers appear wherever display data on a page has been organized
    into a table, and are used extensively throughout the pages available in
    the :ref:`Admin menu <main-menu-admin>`. See: :ref:`Column headers
  ...
  Create button
    ...
    "Save" button, which is used to save all subsequent changes. See:
    :ref:`recurring-button-block`.
  ...
  CSV
    ...
    sometimes called "character-separated values" because the separator
    character does not have to be a comma), CSV is a file format that stores
    tabular data in plain-text form. Information with common properties that
    can be expressed as a sequence of fields sharing a common header can be
    stored in plain-text using the CSV format, making it application agnostic
    and highly transportable.

    A CSV file consists of a number of records (rows), that have identical
    ...
    CSV files and express them as a spreadsheet. See
    `Wikipedia <http://en.wikipedia.org/wiki/Comma-separated_values>`__ for more
    information on CSV. In AtoM, CSV templates conforming to content standards
    templates such as :ref:`ISAD(G) <isad-template>` and
    :ref:`RAD <rad-template>` can be used to migrate large datasets into the
    application. For more information, see: :ref:`csv-import`.
  ...
  Current language
    ...
    languages by using the :term:`language menu`. See: :ref:`language-menu`.
  ...
  Dedicated search box
    ...
    <page-type-browse>`, that will search only one :term:`entity` or record
    type, giving users a means to search for particular content within that
    type. In AtoM, dedicated search bars are provided for: Accessions, Archival
    institutions, authority records, Place terms, Subject terms, Donors,
    Rights holders, Functions, and Users. For more information, see:
  ...
  Delete button
    ...
    really intends to delete the :term:`entity`. See:
    :ref:`recurring-button-block`.
  ...
  Delete icon
    ...
    <access point>`. They are generally portrayed as an "**X**".
  ...
  Duplicate button
    ...
    function in AtoM like a record skeleton when doing large amounts of
    data-entry on similar records. See:
    :ref:`duplicate-archival-description`; see also:
    :ref:`recurring-button-block`.
  ...
  Edit button
    ...
    :term:`title bar` of an :term:`information area`. See:
    :ref:`recurring-button-block`.
  ...
  Facet filter
    ...
    assignment of an object to multiple characteristics (attributes), enabling
    the classification to be ordered in multiple ways, rather than in a
    single, predetermined, taxonomic order.*" (`Wikipedia
    ...
    drawn from the properties of a group of information elements. When applied
    as filters, facets allow a user to access and order query (search or
    browse) results in multiple ways dynamically.

    In AtoM, facet filters are available to users on search and browse pages as
    a means of grouping and narrowing results. They are drawn from available
    :term:`fields <field>` within the :ref:`entity type <entity-types>` being
    browsed or searched. For further information and usage, see:
    :ref:`recurring-facet-filters` and :ref:`settings`. See also:
    :ref:`Browse <browse>`, :ref:`search-atom`, and :ref:`Navigate <navigate>`.

  Facets title

    The facets title is the text that appear above the list of
    :term:`facet filter` headings and results. By default, the text appears as
    "Narrow your results by:". For further information and usage, see:
    :ref:`recurring-facet-filters` and :ref:`settings`. See also:
    :ref:`Browse <browse>`, :ref:`search-atom`, and :ref:`Navigate <navigate>`.
  ...
  Filter tag
    ...
    user arrives there with a search already delimited to a particular
    :term:`collection`, :term:`archival institution`, or other aspect of the
    results (for example, showing only :term:`digital objects <digital
    object>`). Displayed as a white tag with orange text indicating the
    filter being applied (for example, the name of a :term:`repository` to
    which search or browse results have been limited), the filter tag also
    includes a grey "**X**" that, when clicked by a user, will remove the
    filter and reload the page to display global search results (i.e. results
    from throughout all :term:`holdings` in the application).

    The most common uses of filter tags in AtoM are: **1)** When a user clicks
    the "Show results with digital objects" option provided at the top of
    :term:`archival description` search/browse pages, the page will reload
    with a filter tag in place reading, "Only digital objects." **2)** When a
    user performs a search in the :term:`search box` using the
    :ref:`search-box-delimiters`, a search results page with a filter tag for
    that institution will appear (limiting results to the :term:`holdings` of
    the related :term:`repository`). **3)** When a user visits an
    :term:`archival institution` page, and clicks the "Browse # holdings"
    option below the list of the institution's :term:`holdings` in the left-hand
    :term:`context menu`, a browse results page with a filter tag for that
    institution will appear. **4)** When a user performs a :ref:`quick search
    <treeview-search>` on an :term:`archival description` using the
    :term:`treeview` search tab, an option to "Browse all descriptions" is
    presented. If the user clicks the option, AtoM will redirect the user to a
    browse page with a filter tag in place limiting the results to the current
    :term:`fonds` or :term:`collection`. In all cases, clicking the "**X**"
    will remove the filter tag and reload the search results correspondingly.

    See: :ref:`search-atom`; see particularly :ref:`search-box-delimiters`
    for a screenshot and example application of the filter tags.
  ...
  Group profile
    ...
    and each contain their own Group profile: :term:`Administrator`;
  ...
  Hex color code
    ...
    consecutive symbols (a hex triplet) are used to represent the red, green,
    and blue (RGB) components of a color - the first 2 symbols represent the
    red value, the next two (3 and 4) represent the green value, and the last
    two (5 and 6) represent the blue value. Each hex triplet has a value
    range between 00 and FF, and when added together, a hex color code can
    represent a total of 16,777,216 colors. For example, pure black is
    represented in hex color code as 00 00 00 or `#000000`, while pure white
  ...
  Main menu
    ...
    areas of AtoM. See: :ref:`main-menu`
  ...
  Multi-repository system
    ...
    institution>` or other types of :term:`repositories <repository>`.
  ...
  Permissions tab
    ...
    profiles <user profile>` and :term:`group profiles <group profile>`. An
    administrator can view and maintain user and group :term:`access privileges
    <access privilege>` by navigating through these tabs. When editing a user's
    access privileges, these tabs will appear under the user's name as follows:
    "Archival description permissions"; "Authority records permissions";
    "Taxonomy permissions"; and "Archival institution permissions". When editing
    a group's access privileges, these tabs will appear under the user group's
    name as follows: "Information object permissions"; "Actor permissions";
    "Taxonomy permissions"; and "Archival institution permissions".
  ...
  Quick links menu
    ...
    corner of the page, allows users to navigate to :term:`static pages
    <static page>` and/or other external links added to the application by an
    :term:`administrator`. See: :ref:`quick-links-menu`.
  ...
  Repository
    ...
    organizations. For instructions on working with repositories in AtoM,
    see: :ref:`archival-institutions`.
  ...
  Site description
    ...
    of an AtoM installation. When made visible via AtoM's :ref:`Settings
    <settings>`, the site title appears at the top of the
  ...
  Slug
    ...
    *www.yourwebpage.com/About*, the slug is */About*). They are meant to be
    ...
    page` in AtoM **cannot** be edited. New static page slugs can either be
    customized by users or automatically generated by AtoM if the field is left
    blank; AtoM will automatically generate a slug that is based on the "Title"
    you have indicated for the new static page. Note that, it is "common
    practice to make the slug all lowercase, accented characters are usually
    replaced by letters from the English alphabet, punctuation marks are
    generally removed, and long page titles should also be
    truncated to keep the final URL to a reasonable length." (`Wikipedia
    <http://en.wikipedia.org/wiki/Clean_URL#Slug>`__).

  Sort button
    The sort button is a navigational element that appears on some browse and
    search pages throughout AtoM. It allows a user to change the sort order of
    the results being displayed, from alphabetic to most recent. When the sort
    order is set to "Alphabetic," results are organized A-Z. When results are
    ordered by "Most recent" the most recently added and/or edited records in
    the results will appear at the top of the list, allowing users to discover
    new or recently updated content. See: :ref:`recurring-sort-button`.
  ...
  Subject
    ...
    descriptions <archival description>`. See: :ref:`terms`.
  ...
  Title bar
    ...
    on various different :ref:`page types <page-types>`, offering the user an
    indication of the type of page, and/or the name of the record, currently
    being viewed. On a :term:`view <view page>` or :term:`edit <edit page>`
    page, the :term:`title bar` displays the name (title) of the current entity.
    The title bar appears at the top of core :ref:`entity <entity-types>`
    ...
    storage` locations. On Donor and :term:`accession <accession record>`
    records, the title bar displays a message indicating whether the record
    is in :term:`view <view mode>` or :term:`edit <edit mode>` mode, with the
    record's name/title display below (as a sub-title). On :term:`archival
    descriptions <archival description>`, the title bar also displays the
    :term:`level of description` of the displayed description. See:
  ...
  Typeahead
    ...
    with results divided into :ref:`entity types <entity-types>` such as
  ...
  User account
    ...
    See :ref:`user-roles`, :ref:`manage-user-accounts`, and
  ...
  Username
    ...
    of the :term:`user account`. See :ref:`manage-user-accounts`.
  ...
  User role
    ...
    :ref:`User roles <user-roles>`, :ref:`manage-user-accounts`, and
    :ref:`edit-user-permissions`.
  ...
  Visible elements
    ...
    while an unchecked box hides the field. See :ref:`Visible elements
    <visible-elements>`.
